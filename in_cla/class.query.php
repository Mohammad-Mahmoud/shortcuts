...
class query
{
	...
	static function checkGet($q)
	{
		self::numRows($q);
		if (query::$num == 0) {
			echo '<script>';
			echo 'window.location.href = "notfound.php";';
			echo '</script>';
		}
	}
	...
	static function encryptGet($s) {
	    $val = self::generateRandomString(8).$s.md5($s);
	    return $val;
    }

    static function decryptGet($s) {
        $val = substr($s, 8, 1);
        return $val;
    }
}
